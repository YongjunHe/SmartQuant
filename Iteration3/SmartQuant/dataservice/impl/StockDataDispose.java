...
//		
//		try {
//			service = DataFactory.create().getStockService_RMI();
//		} catch (RemoteException e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		}
...
//
//		return stocks;
		Map<String, Stock> stocks = helper.getSelectStockList();
		return stocks;
	}


    public String getCName(String name){
    	return helper.getCName(name);
    }
...
//		return dateMessage;
	...
	//连接数据库数据
		...
		idNumber = idProcess(idNumber);
		Stock stock = helper.selectStock(idNumber, beginDate, endDate);
		List lists = stock.getDateMessage();
	...
	public static void main(String[]args){
		...
		StockDataDispose dispose = new StockDataDispose();
//		Map<String, Stock> map = null;
//		try {
//			map = dispose.getSelectStockList();
//		} catch (Exception e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		}
...
//			System.out.println(stock.getCName() +" "+ stock.getTodayData().getDate() +" " +stock.getTodayData());
//		
//		}
		
		System.out.println(dispose.getCName("sh600000"));
		System.out.println(dispose.getStockMessageWithCName("浦发银行").next().getTodayData());
		
		Iterator iterator = null;
		try {
			iterator = dispose.getStockMessage("sh600000", "2016-05-01", "2016-05-31").iterator();
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		while(iterator.hasNext()){
			StockDateNode stock =  (StockDateNode) iterator.next();
	...
	public Iterator<Stock> getStockMessageWithCName(String name) {
		...
		return helper.selectStockWithCname(name).iterator();
	}
	
	
	private String idProcess(String idNumber){
		String result = null;
		if(Integer.valueOf(idNumber) >= 600000)
			result = "sh" + idNumber;
		else
			result =  "sz" + idNumber;
		return result;
